...
const SignUp = (props) => {
    ...
    const [fname, fnameChangeHandler] = useState('');
    const [lname, lnameChangeHandler] = useState('');
    ...
    const handlerSubmit = (e) => {
        ...
        AuthService.SignUp(result).then(res => {
            if (res.status === 201) {
                props.history.push('/');
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} sm={6}>
                            <TextField
                                ...
                                autoFocus
                                value={fname}
                                onChange={({ target: {value}}) => fnameChangeHandler(value)}
                            />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                            <TextField
                                ...
                                autoComplete="lname"
                                value={lname}
                                onChange={({ target: {value}}) => lnameChangeHandler(value)}
                            />
                        </Grid>
                        <Grid item xs={12}>
                            <TextField
